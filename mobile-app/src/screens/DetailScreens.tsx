...
export const PlanDetailsScreen = ({ navigation, route }: P<'PlanDetails'>) => {
  const { theme } = useAppTheme(); const { plans, hasActivePackage } = useCatalog();
  const plan = plans.find(item => item.id === route.params.planId);
  if (!plan) return <Shell><AppHeader title="Package" back onBack={() => navigation.goBack()}/><EmptyState title="This package is not available."/></Shell>;
  const active = hasActivePackage(plan.id);
  return <Shell><AppHeader title={plan.name} subtitle="Program details" back onBack={() => navigation.goBack()}/><GlassCard><Text style={[s.price, { color: theme.text }]}>₹{plan.price}<Text style={{ fontSize: 16, color: theme.muted }}> / {plan.duration}</Text></Text><Text style={{ color: theme.muted, marginTop: 5 }}>{plan.description}</Text></GlassCard><Text style={[s.heading, { color: theme.text }]}>What’s included</Text>{plan.benefits.map(item => <View key={item} style={s.check}><Ionicons name="checkmark-circle" size={22} color={theme.accent}/><Text style={{ color: theme.text, fontWeight: '600' }}>{item}</Text></View>)}{active && <StatusBadge label="Your active package" tone="success"/>}<AppButton disabled={active} title={active ? 'Already active' : 'Continue to dummy checkout'} onPress={() => navigation.navigate('Checkout', { planId: plan.id })}/></Shell>;
};

export const CheckoutScreen = ({ navigation, route }: P<'Checkout'>) => {
  const { theme } = useAppTheme(); const { plans, activateDummySubscription } = useCatalog();
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const plan = plans.find(item => item.id === route.params.planId);
  if (!plan) return <Shell><AppHeader title="Checkout" back onBack={() => navigation.goBack()}/><EmptyState title="This package is not available."/></Shell>;
  const complete = async () => { setBusy(true); setError(''); try { await activateDummySubscription(plan.id); navigation.replace('PaymentSuccess', { planId: plan.id }); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to activate this package.'); } finally { setBusy(false); } };
  return <Shell><AppHeader title="Dummy checkout" subtitle="Review your test order." back onBack={() => navigation.goBack()}/><GlassCard><View style={s.row}><View style={{ flex: 1 }}><Text style={[s.heading, { color: theme.text }]}>{plan.name}</Text><Text style={{ color: theme.muted }}>{plan.duration} access</Text></View><Text style={[s.priceSmall, { color: theme.text }]}>₹{plan.price}</Text></View></GlassCard><GlassCard><Text style={[s.heading, { color: theme.text }]}>Order summary</Text><View style={s.line}><Text style={{ color: theme.muted }}>Package price</Text><Text style={{ color: theme.text }}>₹{plan.price}</Text></View><View style={s.line}><Text style={{ color: theme.muted }}>Payment mode</Text><Text style={{ color: theme.text }}>Test only</Text></View><View style={[s.line, { borderTopWidth: 1, borderColor: theme.border, paddingTop: 14 }]}><Text style={{ color: theme.text, fontWeight: '900' }}>Total</Text><Text style={[s.priceSmall, { color: theme.text }]}>₹{plan.price}</Text></View></GlassCard><View style={s.secure}><Ionicons name="information-circle" size={20} color={theme.accent}/><Text style={{ color: theme.muted, flex: 1 }}>This is a dummy checkout. No real payment will be charged.</Text></View>{!!error && <Text style={{ color: theme.danger, textAlign: 'center' }}>{error}</Text>}<AppButton disabled={busy} title={busy ? 'Activating…' : 'Complete dummy checkout'} icon="checkmark-circle" onPress={complete}/></Shell>;
};

export const PaymentSuccessScreen = ({ navigation, route }: P<'PaymentSuccess'>) => { const { theme } = useAppTheme(); const { plans } = useCatalog(); const plan = plans.find(item => item.id === route.params.planId); return <Shell><View style={s.state}><View style={[s.stateIcon, { backgroundColor: theme.success + '18' }]}><Ionicons name="checkmark" size={45} color={theme.success}/></View><Text style={[s.stateTitle, { color: theme.text }]}>Package activated</Text><Text style={[s.stateCopy, { color: theme.muted }]}>Your {plan?.name ?? 'selected'} package is active. Its Mux videos are now unlocked.</Text><StatusBadge label={`${plan?.name ?? 'Package'} · Active`} tone="success"/></View><AppButton title="Watch workouts" onPress={() => navigation.replace('Main')}/></Shell>; };
export const PaymentFailedScreen = ({ navigation, route }: P<'PaymentFailed'>) => { const { theme } = useAppTheme(); return <Shell><View style={s.state}><View style={[s.stateIcon, { backgroundColor: theme.danger + '18' }]}><Ionicons name="close" size={45} color={theme.danger}/></View><Text style={[s.stateTitle, { color: theme.text }]}>Activation failed</Text><Text style={[s.stateCopy, { color: theme.muted }]}>We couldn’t activate this package. No amount was charged.</Text></View><AppButton title="Try again" onPress={() => navigation.replace('Checkout', { planId: route.params.planId })}/></Shell>; };
